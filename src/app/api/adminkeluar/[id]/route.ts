...
export async function DELETE(
  ...
  try {
    ...
    console.error("Error deleting keluar data:", error);
    return NextResponse.json(
      { message: "Gagal menghapus data keluar." },
      { status: 500 }
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const updatedKeluar = await prisma.keluar.update({
      where: { id },
      data: {
        idUser,
        waktu: waktuUTC,
    ...
    return NextResponse.json(updatedKeluar);
  } catch (error) {
    // console.error("Error updating masuk data:", error as string);
    return NextResponse.json(
      { message: ("Gagal memperbarui data keluar." + error) as string },
      { status: 500 }
    );
  }
}
