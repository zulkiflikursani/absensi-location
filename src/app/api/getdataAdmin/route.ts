import { PrismaClient } from "@prisma/client";
import { NextRequest, NextResponse } from "next/server";
interface TypeBody {
  bulan: string;
  bagian: string;
  // idUser: string;
}

export async function POST(req: NextRequest) {
  const body = await req.json();
  const data: TypeBody = body;
  const prisma = new PrismaClient();

  try {
    const query = `WITH RECURSIVE
    DateSeries AS(
    SELECT
        CAST('${data.bulan}' AS DATE) AS tanggal -- Start date (1st of the month)
    UNION ALL
SELECT
    tanggal + INTERVAL 1 DAY
FROM
    DateSeries
WHERE
    tanggal < LAST_DAY('${data.bulan}') -- End date (last day of the month)
)
SELECT
    u.id AS idUser,
    u.username,
    u.full_name,
    u.bagian,
    ds.tanggal,
    CONCAT(ds.tanggal, ' 07:30:00') AS jam_masuk,
    CONCAT(ds.tanggal, ' 16:00:00') AS jam_pulang,
    MIN(m.waktu) AS waktu_masuk,
    -- Earliest 'masuk' time
    MAX(k.waktu) AS waktu_keluar,
    -- Latest 'keluar' time
    CAST(
        COALESCE(
            TIMESTAMPDIFF(
                MINUTE,
                MIN(m.waktu),
                MAX(k.waktu)
            ),
            0
        ) AS UNSIGNED
    ) AS selisih_menit,
    CAST(
        COALESCE(
            TIMESTAMPDIFF(
                MINUTE,
                CONCAT(ds.tanggal, ' 07:30:00'),
                MIN(m.waktu)
            ),
            0
        ) AS SIGNED
    ) AS terlambat
FROM
    users u
CROSS JOIN DateSeries ds LEFT JOIN masuk m ON
    DATE(ds.tanggal) = DATE(m.waktu) AND m.idUser = u.id
LEFT JOIN keluar k ON
    DATE(ds.tanggal) = DATE(k.waktu) AND k.idUser = u.id
WHERE
    u.id != '2' AND u.bagian = '${data.bagian}'
GROUP BY
    u.id,
    ds.tanggal
ORDER BY
    u.id,
    ds.tanggal`;
    const result = await prisma.$queryRawUnsafe(query);
    console.log(result);
    const processedResult = Array.isArray(result)
      ? result.map((item) => {
          const newItem: { [key: string]: string } = {};
          for (const key in item) {
            const value = item[key];
            if (typeof value === "bigint") {
              newItem[key] = value.toString();
            } else if (typeof value === "string" && /^\d{4}$/.test(value)) {
              // Jika value seperti "2018", buang atau ganti jadi null
              newItem[key] = "";
            } else {
              newItem[key] = value;
            }
          }
          return newItem;
        })
      : [];

    console.log("processedResult", processedResult);
    return NextResponse.json(processedResult);
  } catch (error) {
    return NextResponse.json({ message: "Error fetching data" + error });
  }
}
