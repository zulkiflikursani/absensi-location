...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    CROSS JOIN
        DateSeries ds
    LEFT JOIN
        masuk m ON DATE(ds.tanggal) =DATE(CONVERT_TZ(m.waktu,'+00:00','+08:00')) AND m.idUser = u.id
    LEFT JOIN
        keluar k ON DATE(ds.tanggal) = DATE(CONVERT_TZ(k.waktu,'+00:00','+08:00'))AND k.idUser = u.id
    GROUP BY
        u.id, ds.tanggal
    ORDER BY
        u.id, ds.tanggal`);
